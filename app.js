...
app.get('/home',nocache,function(req,res)
{
  ...
  if((sess.rsiid||sess.rsiAdmin) && sess.mobNo)
  {
    ...
    res.render('home.hbs',{rsiid :sess.rsiAdmin,id:sess.rsiid});
  }
  else
  {
    res.redirect('/');
...
app.get('/contact',nocache, (req, res) => {
  ...
  if((sess.rsiid||sess.rsiAdmin) && sess.mobNo)
  {
    res.render('contact.hbs',{id:sess.rsiid});
  }
  else
  {
    res.redirect('/');
...
app.get('/history',nocache, (req, res) => {
  ...
  if((sess.rsiid||sess.rsiAdmin) && sess.mobNo)
  {
    res.render('history.hbs',{id:sess.rsiid});
  }
  else
  {
    res.redirect('/');
...
app.get('/sports',nocache, (req, res) => {
  ...
  if((sess.rsiid||sess.rsiAdmin) && sess.mobNo)
  {
    res.render('billiards.hbs',{id:sess.rsiid});
  }
  else
  {
    res.redirect('/');
...
app.get('/sanjog',nocache, (req, res) => {
  ...
  if((sess.rsiid||sess.rsiAdmin) && sess.mobNo)
  {
    res.render('sanjog.hbs',{id:sess.rsiid});
  }
  else
  {
    res.redirect('/');
...
app.get('/bar',nocache, (req, res) => {
  ...
  if((sess.rsiid||sess.rsiAdmin) && sess.mobNo)
  {
    res.render('bar.hbs',{id:sess.rsiid});
  }
  else
  {
    res.redirect('/');
...
app.get('/swimming',nocache, (req, res) => {
  ...
  if((sess.rsiid||sess.rsiAdmin) && sess.mobNo)
  {
    res.render('swimming.hbs',{id:sess.rsiid});
  }
  else
  {
    res.redirect('/');
...
app.get('/party',nocache, (req, res) => {
  ...
  if((sess.rsiid||sess.rsiAdmin) && sess.mobNo)
  {
    res.render('partyhall.hbs',{id:sess.rsiid});
  }
  else
  {
    res.redirect('/');
...
app.get('/trinco',nocache, (req, res) => {
  ...
  if((sess.rsiid||sess.rsiAdmin) && sess.mobNo)
  {
    res.render('trincorestuarent.hbs',{id:sess.rsiid});
  }
  else
  {
    res.redirect('/');
...
app.get('/guest',nocache, (req, res) => {
  ...
  if((sess.rsiid||sess.rsiAdmin) && sess.mobNo)
  {
    res.render('guestroom.hbs',{id:sess.rsiid});
  }
  else
  {
    res.redirect('/');
...
app.get('/dateTickets',nocache,(req,res)=>
{
  sess=req.session;
  if(sess.rsiAdmin&&sess.mobNo)
  {
    res.render('datetickets.hbs',{messages:req.flash('dateTickets')});
  }
  else
  {
    res.redirect('/home');
...
app.get('/adminTickets',nocache, (req, res) => {
  sess=req.session;
  res.header('Cache-Control', 'no-cache, private, no-store, must-revalidate, max-stale=0, post-check=0, pre-check=0');
  if(sess.rsiAdmin==='A-007')
  {
  ...
  if(sess.rsiAdmin && sess.mobNo&&sess.adminTickets)
  {
    res.render('myTickets.hbs',{
      ticketDetails: sess.adminTickets,
      rsiid:sess.rsiAdmin});
  }
  else
  {
...
//   console.log("User :"+sess.rsiid+" Mobile :"+mobNo+" Password :"+password);
...
{
  req.flash('summaryDownload','Please Enter The Date!!!');
  res.redirect('/downloadSummary');
}  
});
...
app.post('/addUser',(req,res)=>
{
  ...
  if(rsiID&&mobno&&pass)
  {
    ...
    admin.database().ref('DepCount/'+rsiID).set(newobj);
...
app.post('/deleteUser',(req,res)=>
{
  ...
  .then(()=>
  {
    if(rsiID)
    {
      if(rsiID==='A-007')
      {
        req.flash('deleteUser','Admin Id Cannot Be Deleted!!!');
...
app.post('/dateTickets',(req,res)=>
{
  ...
  let ticketArr = [];
  let seatsArr = [];
  var ref = admin.database().ref('Tickets/A-007');
  ref.once('value',(snapshot)=>{
  snapshot.forEach(childSnapshot=>{
   if(childSnapshot.val().date===date){
    ...
    for(i=0; i<359; i++){
      ...
      sess.adminTickets=ticketArr;
      res.redirect('/adminTickets');
});
}
else
{
  req.flash('dateTickets',"Please Enter The Date!!!");
  res.redirect('/dateTickets');
}
});
///////////////////////////////////////////////////////////////////////////////
...
//   var mobno  = req.body.mobno;
  ...
  if(rsiID&&newmobno)
  {
    auth.changeMobNo(rsiID,newmobno)
    .then(()=>
    {
      console.log("Resolve");
    ...
    {
      console.log("Reject");
      req.flash('numberChanged',"Invalid RSI-ID!!!");
      res.redirect('/changeMobileNumber');
    })
  }
  else
